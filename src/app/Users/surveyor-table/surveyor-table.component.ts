...
@Component({
  selector: 'app-surveyor-table',
  templateUrl: './surveyor-table.component.html',
  styleUrls: ['./surveyor-table.component.css']
})
export class SurveyorTableComponent {
  ...
  loadUsers(): void {
    ...
    this.userService.getUsers(this.currentPage, this.pageSize).subscribe(response => {
      ...
      this.users=this.users.filter(u=>u.role=="Surveyor")
  ...
  DownloadExcel(){
this.userService.GetAllUsers("Surveyor").subscribe(result=>{
